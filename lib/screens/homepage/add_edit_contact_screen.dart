...
import 'package:safeguard/screens/homepage/emergency_contact_setup_screen.dart'; // Ensure this import is correct
import 'package:safeguard/theme/theme.dart';
import 'package:permission_handler/permission_handler.dart'; // Keep if needed for phonebook access in future

class AddEditContactScreen extends StatefulWidget {
  final EmergencyContact? contactToEdit;
  final Function(EmergencyContact)
  onSave; // Callback to pass contact back to parent
...
class _AddEditContactScreenState extends State<AddEditContactScreen> {
  ...
  void _handleSubmit() {
    ...
    // If editing, use the existing ID. If new, ID will be empty and handled by parent.
    final newContact = EmergencyContact(
      id:
          widget.contactToEdit?.id ??
          '', // Pass existing ID or empty string for new
      name: _nameController.text,
      phoneNumber: _phoneController.text,
      relationship: _selectedRelationship,
    );

    widget.onSave(newContact); // Pass the contact back to the parent
    Navigator.pop(context); // Close the screen
  }

  // This function is not directly used in this screen's logic,
  // but kept if there's a future need for direct phonebook access here.
  Future<bool> requestContactsPermission() async {
    if (await Permission.contacts.request().isGranted) {
      return true;
    }
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: Icon(Icons.phone, color: AppColors.white),
                      contentPadding: const EdgeInsets.symmetric(
                        vertical: 12.0,
                      ),
                    ),
