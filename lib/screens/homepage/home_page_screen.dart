...
import 'package:flutter/material.dart';
import 'package:safeguard/components/layout.dart';
import 'package:safeguard/screens/homepage/emergency_contact_setup_screen.dart'; // Import EmergencyContact and EmergencyContactSetupScreen
import 'package:safeguard/screens/homepage/add_edit_contact_screen.dart';
import 'package:safeguard/service/emergency_service.dart'; // Assuming this handles location/SMS/call
import 'package:safeguard/theme/theme.dart';
import 'package:safeguard/widgets/custom_button.dart';
...
class HomeScreen extends StatefulWidget {
  // Removed emergencyContacts from constructor, will fetch from Firestore
...
class _HomeScreenState extends State<HomeScreen> {
  List<EmergencyContact> _emergencyContacts = []; // Now managed internally
  bool _locationSharingEnabled = true;
  bool _isEmergencyActive = false;
  Map<String, dynamic>? _currentLocation;
  bool _isLoadingLocation = false;
  ...
  Future<void> _fetchEmergencyContacts() async {
    ...
    try {
      ...
      debugPrint('Error fetching emergency contacts for Home Screen: $e');
      if (mounted) {
        ScaffoldMessenger.of(
          context,
  ...
  void _navigateToManageContacts() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const EmergencyContactSetupScreen(),
      ),
    ).then((_) {
      // Refresh contacts after returning from the setup screen
  ...
  void _addNewContact() {
    Navigator.push(
          ...
          onSave: (newContact) async {
            ...
            try {
              DocumentReference docRef = await FirebaseFirestore.instance
                  ...
                  .doc(user.uid)
                  .collection('emergencyContacts')
                  .add(newContact.toFirestore());
              newContact.id = docRef.id; // Assign the Firestore generated ID
              setState(() {
                _emergencyContacts.add(newContact);
              });
              if (mounted) {
              ...
              debugPrint('Error adding new contact from Home Screen: $e');
              if (mounted) {
      ...
      // Refresh contacts after returning from adding a new contact
      _fetchEmergencyContacts();
    });
  }
